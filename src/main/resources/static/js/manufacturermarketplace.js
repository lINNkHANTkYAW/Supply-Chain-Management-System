    ...
    fetch(`/api/manu/products/search?query=${encodeURIComponent(query)}`)
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                ...
                data.forEach(item => {
                    if (!item.id || !item.manufacturerId || !item.companyName || !item.name || !item.price || !item.description) {
                    ...
                    itemsContainer.innerHTML += `
                        <div class="col-md-4">
							...
							<img src="${product.image ? '/uploads/' + product.image : 'img/default-product.jpg'}" 
														                 class="card-img-top" 
														                 style="height: 150px; object-fit: cover;" 
														                 alt="${product.name}">                                           <div class="card-body">
            ...
            <!-- Main Product Details -->
                <div class="col-md-9">
                    <div class="d-flex">
                        ...
                        <div id="productDetails" style="width: 50%; margin-left:10%;">
                            <h4>${product.name}</h4>
                            <p>${product.description || 'No description available'}</p>
                            
                            <button class="btn btn-success mt-3" data-manufacturer-user-id="${manufacturer.manuUserId}" onclick="openChatWithManufacturer(this)">
                                  chat
                            </button>
                        </div>
                    </div>
...
function openChatWithManufacturer(button) {
    const manuUserId = button.getAttribute("data-manufacturer-user-id");
    console.log("Opening chat with manufacturer user ID:", manuUserId);
    window.openChat(manuUserId, "Manufacturer"); // Call the global openChat from distriChat.js
}
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                ...
                limitedProducts.forEach((item, index) => {
                    const productId = item.productMid;
                    const manufacturerId = item.manufacturer?.manufacturerId;
                    const manufacturerName = item.manufacturer?.companyName;
                    const productName = item.name;
                    const productPrice = item.price;
                    const productDescription = item.description;

                    // Validation
                    if (!productId || !manufacturerId || !manufacturerName || !productName || !productPrice || !productDescription) {
                        console.warn(`Skipping item at index ${index} with missing fields:`, item);
                        return;
                    }

                    const manufacturerLink = `/customerseedistributor?distributorId=${manufacturerId}`;
                    const itemCard = `
                        <div class="col-md-4 mb-4">
							...
							<img src="${item.image ? '/uploads/' + item.image : 'img/default-product.jpg'}" 
														                 ...
														                 style="height: 150px; object-fit: cover;" 
														                 alt="${item.name}">                                           <div class="card-body">
                                    <h5 class="card-title">${manufacturerName}</h5>
                                    <h6 class="card-title">${productName}</h6>
                                    <p class="card-text">$${productPrice}</p>
                                    <button class="btn btn-primary btn-sm" onclick="viewDetails(${productId})">Details</button>
                                    <button class="btn btn-secondary btn-sm">
                                        <a href="${manufacturerLink}" style="color: white; text-decoration: none;">Profile</a>
                                    </button>
                                </div>
